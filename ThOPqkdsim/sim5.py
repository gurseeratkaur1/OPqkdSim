...
class Channel:
    ...
    def __init__(self, efficiency):
        """
        Initialize the channel with a specified efficiency.
        
        Args:
            efficiency (float): Channel transmission efficiency (0-1)
        """
...
class BB84Simulator:
    ...
    def __init__(self, mu, alice_detector_efficiency, bob_detector_efficiency, 
                 ...
                 dark_count_rate, time_window, p_eve=0.0):
        """
        ...
        Args:
            ...
            alice_channel_efficiency (float): Efficiency of channel to Alice
            bob_channel_efficiency (float): Efficiency of channel to Bob
            dark_count_rate (float): Dark count rate in counts per second
            time_window (float): Detection time window in seconds
            p_eve (float): Probability of Eve performing intercept-resend attack
        """
        self.source = PhotonSource(mu)
        self.alice_channel = Channel(alice_channel_efficiency)
        self.bob_channel = Channel(bob_channel_efficiency)
...
def plot_qber_vs_mu(mu_values=None, time_window=1e-9,
                   alice_detector_efficiency=0.20,  bob_detector_efficiency=0.20,
                   alice_channel_efficiency=1.00,bob_channel_efficiency=0.1913, 
    ...
    plt.axhline(y=7, color='r', linestyle='--', label='7% threshold')
    plt.grid(True)
    plt.xlabel('Mean Photon Number (μ)', fontsize=12)
    plt.ylabel('QBER (%)', fontsize=12)
    plt.title('Quantum Bit Error Rate vs Mean Photon Number', fontsize=14)
    plt.ylim(0, 15)
...
def plot_skr_vs_mu(mu_values, time_window=1e-9,
                   alice_detector_efficiency=0.20,  bob_detector_efficiency=0.20,
                   alice_channel_efficiency=1.00,bob_channel_efficiency=0.1913, 
                   dark_count_rate=1000, key_length=1000000):
    """
    ...
    plt.grid(True)
    plt.xlabel('Mean Photon Number (μ)', fontsize=12)
    plt.ylabel('Secret Key Rate (bits)', fontsize=12)
